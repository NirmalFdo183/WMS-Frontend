...
const Returns = () => {
  ...
  const filteredLoadings = loadings.filter(
    (l) =>
      l.load_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
      l.truck?.licence_plate_no
        .toLowerCase()
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col h-[calc(100vh-250px)]">
            ...
            <div className="overflow-y-auto flex-1 p-2 space-y-2">
              {loadingError ? (
                <p className="text-red-500 text-center p-4 text-sm">
                  {loadingError}
                </p>
                ...
                filteredLoadings.map((load) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span
                        className={`text-[10px] uppercase font-black px-2 py-0.5 rounded-full ${load.status === "delivered" ? "bg-green-100 text-green-700" : load.status === "pending" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-600"}`}
                      >
                        {load.status}
                      </span>
                    </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            ...
            >
              <RefreshCw
                size={18}
                className={returnsLoading ? "animate-spin" : ""}
              />
            </button>
          </div>
                  ...
                  returnsList.map((ret: any) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-blue-600 font-bold">
                        #{ret.loading?.load_number}
                      </td>
                      <td className="px-6 py-4">
                        <p className="font-bold text-gray-800">
                          {ret.batch_stock?.product?.name}
                        </p>
